...
const GITIGNORE_COMMENT: &str = "# Cloud Code local artifacts";
...
pub(crate) fn render_init_claw_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLOUD.md".to_string(),
        String::new(),
        "This file provides guidance to Cloud Code when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Do not overwrite existing `CLOUD.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains("CLOUD.md         created"));
        assert!(root.join(".claw").is_dir());
        assert!(root.join(".claw.json").is_file());
        assert!(root.join("CLOUD.md").is_file());
        assert_eq!(
        ...
        let claw_md = fs::read_to_string(root.join("CLOUD.md")).expect("read claw md");
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("CLOUD.md"), "custom guidance\n").expect("write existing claw md");
        fs::write(root.join(".gitignore"), ".claw/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("CLOUD.md         skipped (already exists)"));
        ...
        assert!(second_rendered.contains("CLOUD.md         skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("CLOUD.md")).expect("read existing claw md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
